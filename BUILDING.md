...
## はじめに
satpg は ymtools と同様に cmake を用いて Makefile の生成を行います．
...
- tclreadline
  readline (curses) ライブラリを利用してコマンド行の編集機能を強化する Tcl の拡張パッケージです．
  本体のリンク時にリンクすることもできますが，tclsh (atpg_tclsh) の実行時に `package requre` コマンド
  で読み込むこともできます．
  とにかくあると便利です．
