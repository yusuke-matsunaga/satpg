...
///
/// Copyright (C) 2005-2011, 2013-2014, 2015, 2017 Yusuke Matsunaga
/// All rights reserved.
...
FaultAnalyzer::FaultAnalyzer(const TpgNetwork& network,
			     TvMgr& tvmgr) :
  mNetwork(network),
  mTvMgr(tvmgr),
  mVerbose(0),
  ...
  mInputListArray(mMaxNodeId),
  mInputList2Array(mMaxNodeId),
  mFaultInfoArray(mMaxFaultId)
{
}
...
// @brief テストベクタのマネージャを得る．
TvMgr&
FaultAnalyzer::tvmgr() const
{
  return mTvMgr;
}
...
void
FaultAnalyzer::init(vector<const TpgFault*>& fault_list)
{
  ...
  for (ymuint i = 0; i < nn; ++ i) {
    ...
    for (ymuint j = 0; j < nf; ++ j) {
      ...
      SatBool3 stat = analyze_fault(fault);
      ++ f_all;
      switch ( stat ) {
      case kB3True:
...
FaultAnalyzer::analyze_fault(const TpgFault* fault)
{
  ...
  if ( sat_stat == kB3True ) {
    ...
    NodeValList& pi_suf_list = fi.mPiSufficientAssignment;
    {
      ...
      BackTracer backtracer(2, FaultType::StuckAt, mMaxNodeId);
...
FaultAnalyzer::clear_fault_info(ymuint fid)
{
  ...
  mTvMgr.delete_vector(fi.mTestVector);
}
...
{
  const TpgFault* fault = this->fault(fid);
  const TpgNode* node = fault->tpg_onode();
  const TpgNode* ffr_root = node->ffr_root();
